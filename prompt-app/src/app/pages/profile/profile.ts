...
@Component({
  ...
  standalone: false, // Modüler bileşen
...
export class Profile implements OnInit {
  ...
  // ⭐⭐⭐ TOTP ile ilgili değişkenler sadece durum kontrolü için kaldı ⭐⭐⭐
  ...
  ngOnInit(): void {
    ...
    this.authService.getMe().pipe(first()).subscribe({
      next: (user) => {
        if (user) {
          ...
          this.checkTotpStatus(); // Kullanıcı bilgileri yüklendikten sonra TOTP durumunu kontrol et
        } else {
  ...
  // ⭐⭐⭐ TOTP Durum Kontrolü ve Yönlendirme Metodu (QR kodu burada üretilmiyor) ⭐⭐⭐
  ...
  // TOTP Yönetim sayfasına yönlendirme metodu
  navigateToTotpManagement(): void {
    this.router.navigate(['/totp-verify']); // totp-verification sayfasına yönlendiriyoruz
  }
}
